...
{
	// n이 0이면 이동할 원판이 없으므로 종료한다
	if (n == 0)
	{
		return;
	}
	
	// 6 - a - b는 a와 b를 제외한 나머지 기둥의 번호 (임시 기둥)
	// a에서 임시 기둥으로 n - 1개의 원판을 이동한다
	hanoi(a, 6 - a - b, n - 1);
	// 가장 큰 원판을 a에서 b로 이동한다
	cout << a<< ' ' << b << "\n";
	// 임시 기둥에서 b로 n - 1개의 원판을 이동한다
	hanoi(6 - a - b, b, n - 1);
...
void program()
{
	...
	for (int k = 0; k < N; k++)
	{
		...
		if(c > 0)
		{
			// c가 남아있으면 begin에 c를 추가한다
			s.insert(s.begin(), c + '0');
		}
	}

	// 끝자리가 0이면 1을 앞 자리에서 빌려와야 한다
	int i = s.size() - 1;
